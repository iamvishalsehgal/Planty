...
const SEVERITY_CONFIG = {
  water: { border: "border-l-sky-400", bg: "bg-sky-50/50", dot: "bg-sky-400" },
  sun: { border: "border-l-soil-400", bg: "bg-soil-50/50", dot: "bg-soil-400" },
  critical: { border: "border-l-clay-400", bg: "bg-clay-50/50", dot: "bg-clay-400" },
  disease: { border: "border-l-sage-400", bg: "bg-sage-50/50", dot: "bg-sage-400" },
  light: { border: "border-l-soil-400", bg: "bg-soil-50/50", dot: "bg-soil-400" },
};
    ...
    return (
      <div className="flex flex-col h-full animate-page-in">
        <div className="px-5 pt-8 pb-4">
          <h1 className="text-display-lg text-text-primary tracking-tight leading-none">Plant Doctor</h1>
        ...
        <div className="flex-1 overflow-auto px-5 pb-8 space-y-4">
          <GlassCard variant="lg" className="flex flex-col items-center justify-center py-14 gap-4">
            ...
            <p className="text-[14px] text-text-tertiary text-center px-4 max-w-xs leading-relaxed">
          ...
          <div className="space-y-2">
            ...
            {TIPS.map((tip) => {
              ...
              return (
                <div
                  key={tip.problem}
                  className={`p-4 rounded-xl border-l-4 ${sev.border} ${sev.bg} border border-cream-200/40 flex items-center gap-3`}
                >
                  <span className={`w-2.5 h-2.5 rounded-full ${sev.dot} flex-shrink-0`} />
                  <div>
                    <span className="text-[14px] font-semibold text-text-primary block">{tip.problem}</span>
                    <span className="text-[13px] text-text-tertiary">{tip.cause}</span>
                  </div>
                </div>
              );
            })}
